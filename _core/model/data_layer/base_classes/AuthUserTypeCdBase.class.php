...
* - __set()
* Classes list:
* - AuthUserTypeCdBase extends MLCBaseEntity
*/
/**
 * Class Competition
 ...
 */
class AuthUserTypeCdBase extends MLCBaseEntity {
    const DB_CONN = 'DB_0';
    const TABLE_NAME = 'AuthUserTypeCd_tpcd';
    const P_KEY = 'idUserTypeCd';
    ...
    public static function LoadById($intId) {
        return self::Query('WHERE AuthUserTypeCd_tpcd.idUserTypeCd = ' . $intId, true);
    }
    ...
    public function Materilize($arrData) {
        if (isset($arrData) && (sizeof($arrData) > 1)) {
            if ((array_key_exists('AuthUserTypeCd_tpcd.idUserTypeCd', $arrData))) {
                //New Smart Way
                $this->arrDBFields['idUserTypeCd'] = $arrData['AuthUserTypeCd_tpcd.idUserTypeCd'];
                $this->arrDBFields['shortDesc'] = $arrData['AuthUserTypeCd_tpcd.shortDesc'];
                //Foregin Key
    ...
    public static function GetSQLSelectFieldsAsArr($blnLongSelect = false) {
        $arrFields = array();
        $arrFields[] = 'AuthUserTypeCd_tpcd.idUserTypeCd ' . (($blnLongSelect) ? ' as "AuthUserTypeCd_tpcd.idUserTypeCd"' : '');
        $arrFields[] = 'AuthUserTypeCd_tpcd.shortDesc ' . (($blnLongSelect) ? ' as "AuthUserTypeCd_tpcd.shortDesc"' : '');
        return $arrFields;
    }
    public static function Query($strExtra = null, $mixReturnSingle = false, $arrJoins = null) {
        ...
        if (!is_null($strExtra)) {
            $strSql = sprintf("SELECT %s FROM AuthUserTypeCd_tpcd %s %s;", $strFields, $strJoin, $strExtra);
        ...
        if ((is_object($mixReturnSingle)) && ($mixReturnSingle instanceof MLCBaseEntityCollection)) {
            ...
            $collReturn->SetQueryEntity('AuthUserTypeCd');
        }
        if (!is_null($strExtra)) {
            $collReturn->AddQueryToHistory($strSql);
            while ($data = mysql_fetch_assoc($result)) {
                $tObj = new AuthUserTypeCd();
    ...
    public static function Parse($mixData, $blnReturnId = false) {
        if (is_numeric($mixData)) {
            ...
            return AuthUserTypeCd::Load($mixData);
        } elseif ((is_object($mixData)) && (get_class($mixData) == 'AuthUserTypeCd')) {
            if (!$blnReturnId) {
                return $mixData;
            }
            return $mixData->intIdUserTypeCd;
        } elseif (is_null($mixData)) {
            return null;
        } else {
            throw new Exception(__FUNCTION__ . ' - Parameter 1 must be either an intiger or a class type "AuthUserTypeCd"');
        }
    }
    public static function LoadSingleByField($strField, $mixValue, $strCompairison = '=') {
        ...
        $strExtra = sprintf(' WHERE AuthUserTypeCd_tpcd.%s %s %s', $strField, $strCompairison, $strValue);
