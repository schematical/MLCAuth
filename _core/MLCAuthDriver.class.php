...
class MLCAuthDriver{
    ...
    public static function GetRolls($strRollType = null, $objUser = null){
        ...
        if(is_null($objUser)){
            return new MLCBaseEntityCollection();
        }

        $strQuery = sprintf('WHERE AuthRoll.idAuthUser = %s', $objUser->IdUser);

        if(!is_null($strRollType)){
            $strQuery .= sprintf(' AND AuthRoll.rollType = "%s"', $strRollType);
        }
    ...
    public static function GetRollByEntity(MLCBaseEntity $objEntity, $strRollType = null, $blnAllowInvites = false){
        $strQuery = sprintf(
            'WHERE AuthUser.idAuthUser = %s AND AuthUser.idEntity = %s AND AuthUser.entityType = "%s"',
            self::IdUser(),
            $objEntity->getId(),
            get_class($objEntity)
        );
        if(is_null($strRollType)){
            $strQuery .= sprintf(' AND AuthUser.rollType = "%s"', $strRollType);
        }
        if(!$blnAllowInvites){
            $strQuery .= ' AND AuthUser.idAuthUser IS NOT NULL ';
        }
    ...
    public static function GetAuthRollFromQS(){
        $strInviteKey = MLCApplication::QS(MLCAuthQS::invite_token);
        if(!is_null($strInviteKey)){
            $objAuthRoll = AuthRoll::Query(
                sprintf(
                    'WHERE AuthRoll.inviteToken = "%s" AND AuthRoll.idAuthUser IS NULL',
                    $strInviteKey
                ),
                true
            );
