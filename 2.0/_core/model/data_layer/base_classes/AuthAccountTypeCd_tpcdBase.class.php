...
class AuthAccountTypeCd_tpcdBase extends BaseEntity {
    ...
    public static function LoadById($intId) {
        $sql = sprintf("SELECT * FROM %s WHERE idAccountTypeCd = %s;", self::TABLE_NAME, $intId);
    ...
    public static function Parse($mixData, $blnReturnId = false) {
        if (is_numeric($mixData)) {
            ...
            return AuthAccountTypeCd_tpcd::Load($mixData);
        } elseif ((is_object($mixData)) && (get_class($mixData) == 'AuthAccountTypeCd_tpcd')) {
            if (!$blnReturnId) {
                return $mixData;
            }
            ...
            throw new Exception(__FUNCTION__ . ' - Parameter 1 must be either an intiger or a class type "AuthAccountTypeCd_tpcd"');
        }
    }
